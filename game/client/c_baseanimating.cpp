...
void C_BaseAnimating::ProcessMuzzleFlashEvent()
{
	...
	if ( muzzleflash_light.GetBool() )
	{
		...
		if ((m_Attachments.Count() > 0) && (gpGlobals->curtime >= m_flLastMuzzleFlashLightTime))
		{
			...
			GetAttachment( iAttachment, vAttachment, angles ); // set 1 instead "attachment"
...
void C_BaseAnimating::DoAnimationEvents( CStudioHdr *pStudioHdr )
{
	...
	if ( nSeqNum >= nStudioNumSeq )
	{
		...
		DevWarning(2, "%s[%d]: Playing sequence %d but there's only %d in total?\n", GetDebugName(), entindex(), nSeqNum, nStudioNumSeq);
		return;
	}
...
C_BaseAnimating *C_BaseAnimating::CreateRagdollCopy()
{
	...
	C_ClientRagdollGib *pRagdoll = new C_ClientRagdollGib();
	if ( pRagdoll == NULL )
		return NULL;
	...
	if (MyCombatCharacterPointer())
		pRagdoll->m_nGibFlags = MyCombatCharacterPointer()->GetGibFlags();
