...
void CHudVoiceSelfStatus::Paint()
{
   if( !m_pVoiceIcon )
	...
	// Bernt - We want to properly scale the texture:
	surface()->DrawSetColor(GetFgColor());
	surface()->DrawSetTexture(m_pVoiceIcon->textureId);
	surface()->DrawTexturedRect(0, 0, m_pVoiceIcon->Width(), m_pVoiceIcon->Height());
}
...
void CHudVoiceStatus::Paint()
{
	...
	FOR_EACH_LL(m_SpeakingList, i)
	{
		...
		{
			// Bernt - We want to properly scale the texture:
			surface()->DrawSetColor(m_clrIcon);
			surface()->DrawSetTexture(m_pVoiceIcon->textureId);
			surface()->DrawTexturedRect(voice_icon_xpos, ypos + voice_icon_ypos, voice_icon_wide + voice_icon_xpos, ypos + voice_icon_ypos + voice_icon_tall);
		}
