...
static ConVar  cl_interp_npcs("cl_interp_npcs", "0.175", FCVAR_USERINFO, "Interpolate NPC positions starting this many seconds in past (or cl_interp, if greater)");
...
bool C_BaseEntity::CanGlowEntity()
{
	...
	if (GetGlowMode() == GLOW_MODE_TEAMMATE)
	{
		C_BasePlayer *pTeammate = ToBasePlayer(this);
		if (!pTeammate)
			return false;

		if (
			(pLocal == pTeammate) ||
			(pLocal->GetTeamNumber() != pTeammate->GetTeamNumber()) ||
			(pTeammate->GetTeamNumber() <= TEAM_SPECTATOR) ||
			(pLocal->GetTeamNumber() <= TEAM_SPECTATOR) ||
			!pTeammate->IsAlive() ||
			!pLocal->IsAlive()
			)
			return false;
...
Color C_BaseEntity::GetActualGlowColor()
{
	color32 glowColor = m_GlowColor;
	C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
	C_BasePlayer *pThis = ToBasePlayer(this);
	if (pPlayer && pThis)
	{
		if (GetGlowMode() == GLOW_MODE_TEAMMATE)
		{
			if (pPlayer->GetTeamNumber() == pThis->GetTeamNumber()) // A friend.
				glowColor = { 25, 255, 25, 255 };
			else
				glowColor = { 255, 25, 25, 200 }; // An enemy.
		}
		else // Expecting glow mode global...
			glowColor = { 255, 255, 0, 255 };
	}

	return Color(glowColor.r, glowColor.g, glowColor.b, glowColor.a);
}

Vector C_BaseEntity::GetGlowColor()
{
	Color glowColor = GetActualGlowColor();
	Vector vecGlowNormalization = Vector(glowColor.r(), glowColor.g(), glowColor.b());
	vecGlowNormalization /= 255.0f;
	return vecGlowNormalization;
}
...
{
	if ((GetGlowMode() == GLOW_MODE_TEAMMATE) || (GetGlowMode() == GLOW_MODE_GLOBAL))
		return true;

	return false;
}

bool C_BaseEntity::ShouldGlowWhenUnoccluded()
{
	return true;
}
#endif // GLOWS_ENABLE
