			...
			while ( pTemp )
			{
				...
				else
				{
					int iTexLeft	= pTemp->GetInt( "x", 0 ),
						...
						iOrigWidth = pTemp->GetInt("width", 0),
						iOrigHeight = pTemp->GetInt("height", 0);
					...
					for ( int i = 0; i < hudTextureFileRefs.Size(); i++ )
					{
						...
						if ( cszFilename )
						{
							...
							tex->rc.width = iOrigWidth;
							tex->rc.height = iOrigHeight;
...
//-----------------------------------------------------------------------------
// Purpose: Return proper icon bounds : 
//-----------------------------------------------------------------------------

int CHudTexture::GetOrigPosX(void) const
{
	return vgui::scheme()->GetProportionalScaledValue(rc.left);
}

int CHudTexture::GetOrigPosY(void) const
{
	return vgui::scheme()->GetProportionalScaledValue(rc.top);
}

int CHudTexture::GetOrigWide(void) const
{
	return vgui::scheme()->GetProportionalScaledValue(rc.width);
}

int CHudTexture::GetOrigTall(void) const
{
	return vgui::scheme()->GetProportionalScaledValue(rc.height);
}

//-----------------------------------------------------------------------------
// Purpose: Circular Progression
//-----------------------------------------------------------------------------
	...
	for (int i = 0; i < 8; i++)
	{
		if (flEndProgressRadians > Segments[i].minProgressRadians)
		{
			...
			if (flInternalProgress < SEGMENT_ANGLE) // Draw in between
			{
				...
				v[2].m_Position.Init(tx + Segments[i].vert1x * flWide + flDeltaX, ty + Segments[i].vert1y * flTall + flDeltaY);
			...
			{
				v[2].m_Position.Init(tx + flHalfWide + flWide * (Segments[i].vert2x - 0.5), ty + flHalfTall + flTall * (Segments[i].vert2y - 0.5));
				v[2].m_TexCoord.Init(Segments[i].vert2x, Segments[i].vert2y);
			}

			v[1].m_Position.Init(tx + flHalfWide + flWide * (Segments[i].vert1x - 0.5), ty + flHalfTall + flTall * (Segments[i].vert1y - 0.5));
