...
void CViewRender::DrawViewModels( const CViewSetup &view, bool drawViewmodel )
{
	...
	render->Push3DView(viewModelSetup, 0, NULL, GetFrustum(), NULL);
...
void CViewRender::SetupMain3DView( const CViewSetup &view, int &nClearFlags )
{
	...
	{
		render->Push3DView(view, nClearFlags, NULL, GetFrustum(), NULL);
	}
...
void CSkyboxView::Draw()
{
	...
	DrawInternal(VIEW_3DSKY, true, NULL, NULL);
}
...
{
	g_bShouldRenderLocalPlayerExternally = true;
	GameBaseClient->OnLocalPlayerExternalRendering();
	...
	g_bShouldRenderLocalPlayerExternally = false;
	GameBaseClient->OnLocalPlayerExternalRendering();
