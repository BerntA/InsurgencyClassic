...
extern float m_flLastBloodParticleDispatchTime;
...
void BloodImpactCallback( const CEffectData & data )
{
	...
	for (int i = 0; i < ARRAYSIZE(bloodCallbacks); i++)
	{
		if (bloodCallbacks[i].nColor == data.m_nColor)
		{
			...
			// BB2 Proper Gore:
			if (bloodCallbacks[i].nColor == BLOOD_COLOR_RED)
				DispatchParticleEffect(GameBaseShared()->GetSharedGameDetails()->GetBloodParticle(GameBaseClient->IsExtremeGore()), data.m_vOrigin, vecAngles);
			else
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void GibImpactCallback(const CEffectData & data)
{
	bool bExplode = ((data.m_nMaterial >= 1) ? true : false);

	const char *pszHitGroup = "chest";
	switch (data.m_nHitBox)
	{
	...
	C_BaseEntity *pVictim = data.GetEntity();
	if (!pVictim)
		return;
	...
	//if (data.m_nColor == BLOOD_COLOR_RED) - TODO : if we add any npcs which don't want to spawn red blood then add checks for blood type...
	// Spawn blood!
	//
	if (bExplode)
		particleToEmit = GameBaseShared()->GetSharedGameDetails()->GetBloodExplosionMist(GameBaseClient->IsExtremeGore());

	DispatchParticleEffect(particleToEmit, PATTACH_POINT_FOLLOW, pVictim, data.m_nAttachmentIndex);
}

DECLARE_CLIENT_EFFECT("GibImpact", GibImpactCallback);

//-----------------------------------------------------------------------------
// BB2 : Handle Proper Client Side Blood : (only for red blood!)
//-----------------------------------------------------------------------------
void BloodSpraySplatCallback(const CEffectData & data)
{
	...
	for (i = 0; i < cCount; i++)
	{
		...
		UTIL_TraceLine(vecEnd, vecEnd + vecTraceDir * -172, MASK_SOLID_BRUSHONLY & ~CONTENTS_GRATE, data.GetEntity(), COLLISION_GROUP_NONE, &Bloodtr);

		if (Bloodtr.fraction != 1.0)
			UTIL_DecalTrace(&Bloodtr, GameBaseClient->IsExtremeGore() ? "ExtremeBlood" : "Blood");
	}
}

DECLARE_CLIENT_EFFECT("BloodSpraySplat", BloodSpraySplatCallback);
