...
struct LoadingTipItem_t
{
	char pchLoadingTip[1024];
};
...
void INSLoadingDialog::Setup(void)
{
	...
	CUtlVector< LoadingTipItem_t> tipList;
	...
	if (pTips)
	{
		...
		{
			LoadingTipItem_t item;
			Q_strncpy(item.pchLoadingTip, pTip->GetString(), sizeof(item.pchLoadingTip));
			tipList.AddToTail(item);
		}
	}
