...
void CBaseHudChatInputLine::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	m_pInput->SetFgColor( pScheme->GetColor( "Chat.TypingText", pScheme->GetColor( "Panel.FgColor", Color( 0, 0, 0, 255 ) ) ) );
...
void CBaseHudChat::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	Color cColor = pScheme->GetColor( "Blank", GetBgColor() );
...
void CBaseHudChat::MsgFunc_SayText2( bf_read &msg )
{
	...
	if (bWantsToChat)
	{
		ChatPrintf(client, "%s", ansiString);
		Msg("%s\n", RemoveColorMarkup(ansiString));

		CLocalPlayerFilter filter;
		C_BaseEntity::EmitSound(filter, SOUND_FROM_LOCAL_PLAYER, "HudChat.Message");
	}
	else // print raw chat text		
		ChatPrintf(client, "%s", ansiString);
}
...
void CBaseHudChat::StartMessageMode( int iMessageModeType )
{
	...
	int x, y, w, h;
	int txtWide, txtTall;
	m_pChatInput->GetPrompt()->GetContentSize(txtWide, txtTall);

	m_pChatInput->GetBounds(x, y, w, h);
	x += GetXPos() + txtWide + vgui::scheme()->GetProportionalScaledValue(12);
	y += GetYPos() + (h / 2);
...
void CBaseHudChatLine::InsertAndColorizeText( wchar_t *buf, int clientIndex )
{
	...
	Color colCustom;
	if (m_text[0] == COLOR_PLAYERNAME || m_text[0] == COLOR_LOCATION || m_text[0] == COLOR_NORMAL || m_text[0] == COLOR_ACHIEVEMENT || m_text[0] == COLOR_CREEPY || m_text[0] == COLOR_CUSTOM || m_text[0] == COLOR_HEXCODE || m_text[0] == COLOR_HEXCODE_ALPHA)
	{
		while ( txt && *txt )
		{
			TextRange range;
