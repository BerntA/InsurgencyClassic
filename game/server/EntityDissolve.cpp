	...
	float flStartTime, int nDissolveType, bool *pRagdollCreated )
{
	...
	{
		if ( pTarget->IsNPC() && pTarget->MyCombatCharacterPointer() && pTarget->MyCombatCharacterPointer()->CanBecomeRagdoll())
		{
			CTakeDamageInfo info;
			CBaseEntity *pRagdoll = CreateServerRagdoll( pTarget->GetBaseAnimating(), 0, info, COLLISION_GROUP_DEBRIS );
