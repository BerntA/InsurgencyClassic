...
bool CFuncTank::StartControl( CBaseCombatCharacter *pController )
{
	...
	if ( pController->IsPlayer() )
	{
		...
		// BB2 Hands
		pPlayer->Weapon_Switch( pPlayer->Weapon_OwnsThisType( "weapon_hands" ) );//Information: Prevents weapon sounds/effects on throwing picked up objects.
	}
	else
	{
	...
	{
		m_hController->GetActiveWeapon()->FullHolster();
	}
...
void CFuncTank::StopControl()
{
	...
	if ( m_hController->IsPlayer() )
	{
		...
		pPlayer->SwitchToNextBestWeapon(pPlayer->GetActiveWeapon());//Restores to a weapon.
	}
...
void CFuncTank::AimFuncTankAtTarget( void )
{
	...
	else
	{
		if ( m_spawnflags & SF_TANK_LINEOFSIGHT )
		{
			...
			UTIL_TraceLine(barrelEnd, worldTargetPosition, MASK_BLOCKLOS_AND_NPCS, &traceFilter, &tr);
		}
		else
		{
			tr.fraction = 1.0f;
	...
	if ( CanFire() && ( ( (fabs(distX) <= m_pitchTolerance) && (fabs(distY) <= m_yawTolerance) ) || (m_spawnflags & SF_TANK_LINEOFSIGHT) ) )
	{
		...
		if ( m_spawnflags & SF_TANK_LINEOFSIGHT )
		{
			UTIL_TraceLine( barrelEnd, pTarget->WorldSpaceCenter(), MASK_SHOT, this, COLLISION_GROUP_NONE, &tr );
...
void CFuncTankCombineCannon::UpdateBeamThink()
{
	...
	UTIL_TraceLine( vecBarrel, vecBarrel + vecAim * COMBINE_CANNON_BEAM_MAX_DIST, MASK_SHOT, this, COLLISION_GROUP_NONE, &trBeam );
...
void CFuncTankCombineCannon::FuncTankPostThink()
{
	...
	if( m_hTarget.Get() == NULL )
	{
		if( gpGlobals->curtime > m_flTimeNextSweep )
		{
			...
			{
			    // Harrassing doesn't appear to work for hl2mp, so we hack this to work. Basically we dont put any code here for harrass and instead place it in the else below.
				vecTargetPosition = pPlayer->EyePosition();
			}
			else
			{
			// We include our own form of harrassing the player here. Basically if a player isnt in sight, the code runs as normal shooting enemies. If a player comes in sight, then the guns start attacking them till they manage to hide.
