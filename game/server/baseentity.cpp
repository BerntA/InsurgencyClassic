...
#ifdef GLOWS_ENABLE
	// BB2 L4D Glow
	m_iGlowMethod.Set(GLOW_MODE_NONE);
...
BEGIN_DATADESC_NO_BASE( CBaseEntity )
	...
	// BB2
#ifdef GLOWS_ENABLE
	DEFINE_INPUTFUNC(FIELD_INTEGER, "SetGlow", InputGlow),
...
int CBaseEntity::UpdateTransmitState()
{
	...
	// Glow Mode Global = always send!
	if ((GetGlowMode() == GLOW_MODE_GLOBAL) || (GetGlowMode() == GLOW_MODE_TEAMMATE))
	{
		return SetTransmitState(FL_EDICT_ALWAYS);
	}
...
#ifdef GLOWS_ENABLE
//-----------------------------------------------------------------------------
// Purpose: Allow clients to change / add glow color and glowing itself to any entity!
//-----------------------------------------------------------------------------
...
void CBaseEntity::SetGlowMode(int mode)
{
	int currentMode = m_iGlowMethod;
	if ((mode == GLOW_MODE_GLOBAL) || (mode == GLOW_MODE_TEAMMATE))
	...
	if ((mode != GLOW_MODE_GLOBAL) && (mode != GLOW_MODE_TEAMMATE))
	{
		...
		if ((currentMode == GLOW_MODE_GLOBAL) || (currentMode == GLOW_MODE_TEAMMATE))
			DispatchUpdateTransmitState();
	}
}
...
{
	Vector vecOtherOrig = pEntity->GetAbsOrigin();
	Vector vecThisOrig = GetAbsOrigin();

	float flWide = (CollisionProp()->OBBSize().x/2);
	float flLen = (CollisionProp()->OBBSize().y/2);

	float absWide = flWide + vecThisOrig.x;
	float absLen = flLen + vecThisOrig.y;

	if ( ( vecOtherOrig.x > -absWide ) && ( vecOtherOrig.x < absWide ) && ( vecOtherOrig.y > -absLen ) && ( vecOtherOrig.y < absLen ) )
		return true;
