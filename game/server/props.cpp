...
void CBasePropDoor::ChangeLockedState(CBaseEntity *pActivator)
{
	m_bLocked = !m_bLocked;
...
void CBasePropDoor::DoorOpen(CBaseEntity *pOpenAwayFrom)
{
	...
	if ( IsDoorOpen() || IsDoorOpening() || IsDoorLocked() )
		return;
...
//
// Purpose: Breakable door entity.
//
//========================================================================================//

struct BreakableDoorDataGib_t
{
	char pszGib[MAX_WEAPON_STRING];
};

class CBreakableDoorData
{
public:

	CBreakableDoorData(int health, const char *model)
	{
		m_pGibs.Purge();
		iHealth = health;
		...
		Q_strncpy(pszDoorModel, model, MAX_WEAPON_STRING);
		CBaseAnimating::PrecacheModel(model);
	}
	...
	void AddGib(const char *model)
	{
		...
		BreakableDoorDataGib_t pGib;
		Q_strncpy(pGib.pszGib, model, MAX_WEAPON_STRING);
		m_pGibs.AddToTail(pGib);
		CBaseAnimating::PrecacheModel(model);
	}

	bool HasGibs(void)
	{
		return (m_pGibs.Count() >= 1);
	}

	const char *GetDoor(void) { return pszDoorModel; }
	int GetHealth(void) { return iHealth; }

	CUtlVector<BreakableDoorDataGib_t> m_pGibs;

private:
	int iHealth;
	char pszDoorModel[MAX_WEAPON_STRING];
};

class CPropDoorBreakable : public CPropDoorRotating
{
	DECLARE_CLASS(CPropDoorBreakable, CPropDoorRotating);
	DECLARE_DATADESC();
...
private:
	...
	CUtlVector<CBreakableDoorData*> m_pDoorData;
};
...
LINK_ENTITY_TO_CLASS(prop_door_breakable, CPropDoorBreakable);

CPropDoorBreakable::CPropDoorBreakable()
{
	m_clrGlow = { 20, 200, 50, 255 };
	m_GlowColor = m_clrGlow;
	pszScriptFile = NULL_STRING;
	m_iDoorHealth = m_iHealth = m_iMaxHealth = 100;
...
{
	if (pszScriptFile == NULL_STRING)
	{
		Warning("prop_door_breakable '%s' with no door data!\nRemoving!\n", STRING(GetEntityName()));
		UTIL_Remove(this);
		return;
	}

	if (!ParseDoorData(STRING(pszScriptFile)))
	{
		Warning("prop_door_breakable '%s' unable to parse door data!\nRemoving!\n", STRING(GetEntityName()));
		UTIL_Remove(this);
		return;
	}
...
bool CPropDoorBreakable::ParseDoorData(const char *script)
{
	...
	if (pkvData)
	{
		...
		{
			CBreakableDoorData *pDoorData = new CBreakableDoorData(atoi(sub->GetName()), sub->GetString("model"));

			KeyValues *pGibKey = sub->FindKey("gibs");
			if (pGibKey)
			{
				for (KeyValues *subGib = pGibKey->GetFirstSubKey(); subGib; subGib = subGib->GetNextKey())
					pDoorData->AddGib(subGib->GetString());
			}

			m_pDoorData.AddToTail(pDoorData);
		}
...
int CPropDoorBreakable::OnTakeDamage(const CTakeDamageInfo &info)
{
	...
	CTakeDamageInfo dmgCopy = info;
	dmgCopy.ScaleDamage(m_flDamageScale);

	int ret = BaseClass::OnTakeDamage(dmgCopy);
	if (iOldHealth != m_iHealth)
		SpawnGibs(m_iHealth, iOldHealth, dmgCopy);

	return ret;
}

void CPropDoorBreakable::Event_Killed(const CTakeDamageInfo &info)
{
	AddEffects(EF_NODRAW);
...
void CPropDoorBreakable::SpawnGibs(int newHealth, int oldHealth, const CTakeDamageInfo &info)
{
	for (int i = (m_pDoorData.Count() - 1); i >= 0; i--)
	{
		if ((newHealth > 0) && (oldHealth > 0))
		{
			if (newHealth <= m_pDoorData[i]->GetHealth() && m_pDoorData[i]->GetHealth() < oldHealth)
			{
				SetModel(m_pDoorData[i]->GetDoor());
				UpdatePhysics();
				break;
			}
		}
	...
	for (int i = 0; i < m_pDoorData.Count(); i++)
	{
		if (!m_pDoorData[i]->HasGibs())
			continue;

		if ((newHealth <= m_pDoorData[i]->GetHealth() && (oldHealth >= m_pDoorData[i]->GetHealth())))
		{
			bool bDeathGibs = ((newHealth == 0) && (oldHealth == 0));
			if (!bDeathGibs && (m_pDoorData[i]->GetHealth() <= 0))
				continue;

			for (int iProp = 0; iProp < m_pDoorData[i]->m_pGibs.Count(); iProp++)
			{
				int modelIndex = modelinfo->GetModelIndex(m_pDoorData[i]->m_pGibs[iProp].pszGib);
				if (modelIndex)
				{
					CPASFilter filter(WorldSpaceCenter());
					...
					Warning("prop_door_breakable has invalid model index for gib %s!\nIs it properly precached?\n", m_pDoorData[i]->m_pGibs[iProp].pszGib);
			}
		}
	}
}