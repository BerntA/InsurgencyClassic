...
{
public:
	CBulletsTraceFilter(const IHandleEntity *passentity, int collisionGroup, int team);
	virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask);
...
void		UTIL_BloodDrips( const Vector &origin, const Vector &direction, int color, int amount, int hitbox = 0 );
void		UTIL_BloodImpact(const Vector &pos, const Vector &dir, int color, int amount, int hitbox = 0);
void		UTIL_GibImpact(CBaseEntity *pVictim, int attachment, int color, int hitbox = 0, bool bExplode = false);
void        UTIL_BloodSpawn(int iEntIndex, const Vector &pos, const Vector &dir, float flDamage, int hitbox = 0);
