...
void UTIL_BloodDrips(const Vector &origin, const Vector &direction, int color, int amount, int hitbox)
{
	...
	{
		// Normal blood impact
		UTIL_BloodImpact( origin, direction, color, amount, hitbox );
	}
}	
...
//			amount - 
//-----------------------------------------------------------------------------
void UTIL_BloodImpact(const Vector &pos, const Vector &dir, int color, int amount, int hitbox)
{
	CEffectData	data;
...
void UTIL_GibImpact(CBaseEntity *pVictim, int attachment, int color, int hitbox, bool bExplode)
{
	if (!pVictim)
		return;
	...
	data.m_vNormal = pVictim->GetAbsOrigin();
	data.m_flScale = 255.0f;
	data.m_nMaterial = (bExplode ? 1 : 0);

#ifdef CLIENT_DLL
	data.m_hEntity = ClientEntityList().GetEnt(pVictim->entindex());
#else
	data.m_nEntIndex = pVictim->entindex();
#endif 
...
void UTIL_BloodSpawn(int iEntIndex, const Vector &pos, const Vector &dir, float flDamage, int hitbox)
{
	...
	DispatchEffect("BloodSpraySplat", data);
}

void UTIL_ExplosionEffect(const Vector &pos, int explosiveType)
{
	CEffectData	data;
	data.m_vOrigin = pos;
	data.m_vNormal = pos;
	data.m_nAttachmentIndex = explosiveType;
	DispatchEffect("GenericExplosionEffect", data);
}
...
void UTIL_EncryptICE( unsigned char * buffer, int size, const unsigned char *key )
{
	if ( !key )
		return;
