...
bool CGameMovement::IsMovementFrozen(void) const
{
	return (IsDead() || (player->GetFlags() & FL_FROZEN) != 0);
}

bool CGameMovement::IsSideMoveAllowed(void) const
{
	return true;
}
...
void CGameMovement::ProcessMovement( CBasePlayer *pPlayer, CMoveData *pMove )
{
	...
	StartTrackPredictionErrors(pPlayer);
	...
	FinishTrackPredictionErrors(pPlayer);
...
void CGameMovement::CheckWaterJump( void )
{
	...
	trace_t tr;
	TracePlayerBBox( vecStart, vecEnd, PlayerSolidMask(), player->GetCollisionGroup(), tr );
...
void CGameMovement::WaterMove( void )
{
	...
	if ( pm.fraction == 1.0f )
	{
		...
		TracePlayerBBox( start, dest, PlayerSolidMask(), player->GetCollisionGroup(), pm );
...
void CGameMovement::StepMove( Vector &vecDestination, trace_t &trace )
{
	...
	TracePlayerBBox( mv->GetAbsOrigin(), vecEndPos, PlayerSolidMask(), player->GetCollisionGroup(), trace );
...
void CGameMovement::Friction( void )
{
	...
	if (player->GetGroundEntity() != NULL)  // On an entity that is the ground
	{
		...
		control = (speed < GetStopSpeed()) ? GetStopSpeed() : speed;
...
void CGameMovement::StayOnGround( void )
{
	...
	TracePlayerBBox( mv->GetAbsOrigin(), start, PlayerSolidMask(), player->GetCollisionGroup(), trace );
	start = trace.endpos;
	...
	TracePlayerBBox( start, end, PlayerSolidMask(), player->GetCollisionGroup(), trace );
...
int CGameMovement::TryPlayerMove( Vector *pFirstDest, trace_t *pFirstTrace )
{
	...
	for (bumpcount=0 ; bumpcount < numbumps; bumpcount++)
	{
		...
		if ( g_bMovementOptimizations )
		{
			...
			{
				TracePlayerBBox( mv->GetAbsOrigin(), end, PlayerSolidMask(), player->GetCollisionGroup(), pm );
			}
		}
		else
		{
		...
		{	
			if ( numbumps > 0 && pm.fraction == 1 )
			{
				...
				trace_t stuck;
				TracePlayerBBox( pm.endpos, pm.endpos, PlayerSolidMask(), player->GetCollisionGroup(), stuck );
...
float CGameMovement::ClimbSpeed(void)
{
...
bool CGameMovement::LadderMove( void )
{
	...
	TracePlayerBBox( mv->GetAbsOrigin(), end, LadderMask(), player->GetCollisionGroup(), pm );
...
void CGameMovement::PushEntity( Vector& push, trace_t *pTrace )
{
	...
	TracePlayerBBox( mv->GetAbsOrigin(), end, PlayerSolidMask(), player->GetCollisionGroup(), *pTrace );
...
int CGameMovement::CheckStuck( void )
{
	...
	hitent = TestPlayerPosition( mv->GetAbsOrigin(), player->GetCollisionGroup(), traceresult );
	...
	if ( !player->IsServer() )
	{
		if ( MoveHelper()->IsWorldEntity( hitent ) )
		{
			...
			do 
			{
				...
				if ( TestPlayerPosition( test, player->GetCollisionGroup(), traceresult ) == INVALID_ENTITY_HANDLE )
				{
					ResetStuckOffsets( player );
...
void CGameMovement::SetGroundEntity( trace_t *pm )
{
	...
	player->SetGroundEntity(newGround, pm);
...
#define NON_JUMP_VELOCITY 140.0f
	...
	{
		// Try and move down.
		TryTouchGround( bumpOrigin, point, GetPlayerMins(), GetPlayerMaxs(), MASK_PLAYERSOLID, player->GetCollisionGroup(), pm );
		
		// Was on ground, but now suddenly am not.  If we hit a steep plane, we are not on ground
		if ( !pm.m_pEnt || pm.plane.normal[2] < 0.7 )
		{
			// Test four sub-boxes, to see if any of them would have found shallower slope we could actually stand on
			TryTouchGroundInQuadrants( bumpOrigin, point, MASK_PLAYERSOLID, player->GetCollisionGroup(), pm );
