...
//-----------------------------------------------------------------------------
void CalculateBulletDamageForce(CTakeDamageInfo* info, int iBulletType, float flVelocity, const Vector& vecBulletDir, const Vector& vecForceOrigin, float flScale)
{
	...
	VectorNormalize(vecForce);
	vecForce *= (flVelocity * 3.2808399f) * BulletData.m_flPartPhysicsImpulse;
	vecForce *= phys_pushscale.GetFloat();
	vecForce *= flScale;
	info->SetDamageForce(vecForce);
}
...
{
	const CBulletData& BulletData = CBulletDef::GetBulletData(iBulletType);
	CalculateBulletDamageForce(info, iBulletType, BulletData.m_iMuzzleVelocity, vecBulletDir, vecForceOrigin, flScale);
}
...
void GuessDamageForce(CTakeDamageInfo* info, const Vector& vecForceDir, const Vector& vecForceOrigin, float flScale)
{
	...
	{
		CalculateBulletDamageForce(info, BULLET_556NATO, vecForceDir, vecForceOrigin, flScale);
	}
