		...
		)
{
	...
	if( IsSnapshotting() )
	{
		...
		if ( hasBump )
		{
			sdk_vertexlit_and_unlit_generic_bump_vs20_Static_Index vshIndex;
			...
			sdk_vertexlit_and_unlit_generic_bump_ps20_Static_Index pshIndex;
		...
		{
			sdk_vertexlit_and_unlit_generic_vs20_Static_Index vshIndex;
			...
			sdk_vertexlit_and_unlit_generic_ps20_Static_Index pshIndex;
			pshIndex.SetBASETEXTURE( hasBaseTexture );
	...
	else
	{
		...
		if ( hasBump )
		{
			sdk_vertexlit_and_unlit_generic_bump_vs20_Dynamic_Index vshIndex;
			...
			sdk_vertexlit_and_unlit_generic_bump_ps20_Dynamic_Index pshIndex;
		...
		{
			sdk_vertexlit_and_unlit_generic_vs20_Dynamic_Index vshIndex;
			vshIndex.SetLIGHT_COMBO( lightCombo );
			vshIndex.SetFOG_TYPE( fogIndex );
			vshIndex.SetNUM_BONES( numBones );
			s_pShaderAPI->SetVertexShaderIndex( vshIndex.GetIndex() );

			sdk_vertexlit_and_unlit_generic_ps20_Dynamic_Index pshIndex;
			pshIndex.SetWRITEWATERFOGTODESTALPHA( fogType == MATERIAL_FOG_LINEAR_BELOW_FOG_Z &&
				blendType != BT_BLENDADD && blendType != BT_BLEND && !bIsAlphaTested );
			pshIndex.SetDOWATERFOG( fogType == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
			s_pShaderAPI->SetPixelShaderIndex( pshIndex.GetIndex() );
