...
__HrLoadAllImportsForDll(LPCSTR szDll) {
    ...
    if (pinh->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT].Size) {
        ...
        while (pidd->rvaDLLName) {
            ...
            if (cchDllCur == strlen(szDll) && strnicmp(szDll, szDllCur, cchDllCur) == 0) {
...
//deathz0rz [
char* __pszDliSearchPath;
int __iDliAllowFail;

void __DLFail(const char* pszError)
{
...
FARPROC WINAPI __DLFailure(unsigned dliNotify, PDelayLoadInfo pdli)
{
	if (dliNotify==dliFailLoadLib)
	{
		char szMsg[512];
		sprintf(szMsg,"Failed loading DLL %s, error code %d",pdli->szDll,pdli->dwLastError);
		__DLFail(szMsg);
	}
	else
	{
		...
		if (pdli->dlp.fImportByName)
			sprintf(szMsg,"Failed loading DLL %s procedure %s, error code %d",pdli->szDll,pdli->dlp.szProcName,pdli->dwLastError);
		else
			sprintf(szMsg,"Failed loading DLL %s procedure %d, error code %d",pdli->szDll,pdli->dlp.dwOrdinal,pdli->dwLastError);
		__DLFail(szMsg);
	}
	return NULL;
}

FARPROC WINAPI __DLNotice(unsigned dliNotify, PDelayLoadInfo pdli)
{
	if (dliNotify==dliNotePreLoadLibrary)
	{
		char* pszTmp=__pszDliSearchPath+strlen(__pszDliSearchPath);
		strcpy(pszTmp,pdli->szDll);
		HMODULE hTmp=LoadLibraryEx(__pszDliSearchPath,NULL,LOAD_WITH_ALTERED_SEARCH_PATH);
		*pszTmp='\0';
		return (FARPROC)hTmp;
	}
	else
