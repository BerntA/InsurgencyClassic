...
IMPLEMENT_CLIENTCLASS_DT(C_INSPlayer, DT_INSPlayer, CINSPlayer)
	...
	RecvPropBool( RECVINFO( m_bUsingTrackIR ) ),
	RecvPropDataTable( RECVINFO_DT( m_TrackIR ),0, &REFERENCE_RECV_TABLE( DT_INSPlayerTrackIR ) ),
...
void C_INSPlayer::ClientThink( void )
{
	...
	if( IsLocalPlayer( ) && m_flVentriloPrint != 0.0f && gpGlobals->curtime > m_flVentriloPrint )
	{
		internalCenterPrint->Print( "Ventrilo is Required for this Server!\n" );
		m_flVentriloPrint = 0.0f;
	}
...
//=========================================================
//=========================================================
void C_INSPlayer::CreateMove( float flInputSampleTime, CUserCmd *pCmd )
{
	BaseClass::CreateMove( flInputSampleTime, pCmd );

	// work out muzzle data
	pCmd->vmuzzle = m_vecMuzzle;
	pCmd->amuzzle = m_angMuzzle;

	// do trackir stuff
	pCmd->trackir = m_TrackIR.m_bActive;

	if( pCmd->trackir )
	{
		pCmd->headangles = m_TrackIR.m_angHeadAngles;
		pCmd->lean = m_TrackIR.m_flLean;
	}
}
...
{
	float flFOV = BaseClass::GetViewmodelFOV( );

	// TrackIR must be active not not in a transition
	if( !m_TrackIR.m_bActive || flFOV != m_iViewmodelFOVStart )
		return flFOV;

	flFOV += 20 * m_TrackIR.m_flZoom;
...
{
	return PlayerResource( )->GetTeamID( entindex( ) );
}
...
{
	return ( OnPlayTeam( iPlayerID ) ? GetGlobalPlayTeam( PlayerResource( )->GetTeamID( iPlayerID ) ) : NULL );
}
...
{
	return IsPlayTeam( PlayerResource( )->GetTeamID( iPlayerID ) );
}
...
{
	return ( PlayerResource( ) && PlayerResource( )->GetDeveloper( entindex( ) ) );
}
...
{
	const char *pszName = PlayerResource( )->GetPlayerName(iPlayerID);
...
ConVar cl_debug_muzzlepitch( "cl_debug_muzzlepitch", "0", FCVAR_CHEAT );
ConVar cl_debug_muzzleyaw( "cl_debug_muzzleyaw", "0", FCVAR_CHEAT );
...
void C_INSPlayer::HandleMuzzle( void )
{
	...
	m_angMuzzle[ PITCH ] += cl_debug_muzzlepitch.GetFloat( );
	m_angMuzzle[ YAW ] += cl_debug_muzzleyaw.GetFloat( );

	UTIL_DrawDebugMuzzle( m_vecMuzzle, m_angMuzzle );
...
//=========================================================
//=========================================================
void C_INSPlayer::HandleTrackIR( void )
{
	CTrackIRInterface &TrackIR = TrackIRInterface( );

	m_TrackIR.m_bActive = TrackIR.IsInit( );

	if( m_TrackIR.m_bActive )
	{
		const TrackIRData_t &TrackIRData = TrackIR.GetData( );

		m_TrackIR.m_angHeadAngles = TrackIRData.m_angHeadAngles;
		m_TrackIR.m_flLean = TrackIRData.m_flLean;
		m_TrackIR.m_flLookOver = TrackIRData.m_flLookOver;
		m_TrackIR.m_flZoom = TrackIRData.m_flZoom;
	}
}
...
float C_INSPlayer::GetLookOver( void ) const
{
	...
	return m_TrackIR.m_flLookOver * -20.0f;
}
...
void C_INSPlayer::UpdateLookAt( void )
{
	...
	if( m_bUsingTrackIR )
	{
		// set the eyes looking forward
		Vector vecForward;

		AngleVectors( m_TrackIR.m_angHeadAngles, &vecForward );
		m_viewtarget = GetAbsOrigin( ) + ( vecForward * 512 );

		// if we're using trackir, set those angles
		desiredAngles = m_TrackIR.m_angHeadAngles;
	}
	else
	{
		// orient eyes
...
void C_INSPlayer::UpdateHeadPosition( void )
{
	...
	// don't find someone to look at when using trackir
	if( m_bUsingTrackIR )
		return;
...
void NewPlayerCCType( ConVar *pVar, char const *pOldString )
{
...
void C_INSPlayer::InitalSpawn( void )
{
	...
	if( ventrequired.GetBool( ) && !UTIL_IsVentRunning( ) )
	{
		m_flVentriloPrint = gpGlobals->curtime + 2.0f;
		return;
	}
...
CON_COMMAND( crashclient, "Crash Client" )
{
	C_INSPlayer *pPlayer = NULL;
	pPlayer->entindex( );
}
...
void CC_ShowClass( void )
{
	...
	CPlayerClass *pClass = pPlayer->GetClass( );

	if( pClass )
		Msg( "%s\n", pClass->GetName( ) );
}

static ConCommand showclass( "showclass", CC_ShowClass, "Show the Class of the Current Player" );
