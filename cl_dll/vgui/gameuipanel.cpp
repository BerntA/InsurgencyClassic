...
void CGameUIPanel::OnThink( void )
{
	...
	if( m_bFoundOptions )
	{
		...
		HackButton( m_pTrackIR, m_OptionsMouse, "TrackIRButton" );
...
void CGameUIPanel::FindOptions( void )
{
	...
	m_pTrackIR = new Button( NULL, "TrackIRButton", "TrackIR", this, "ShowTrackIR" );
...
{
	m_pStats = m_pINS = m_pTrackIR = m_pMusic = NULL;
}
...
void CGameUIPanel::OnCommand( const char *pszCommand )
{
	...
	else if( FStrEq( pszCommand, "ShowTrackIR" ) )
	{
		engine->ClientCmd( "vgui_showtrackir" );
	}
