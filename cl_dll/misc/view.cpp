...
void CViewRender::SetUpView()
{
	...
	float flFOVOffset = 0.0f;
	
	if( g_pGameRules )
		flFOVOffset = g_pGameRules->DefaultFOV( ) - m_View.fov;
...
void CViewRender::DrawZoomed( const CViewSetup &zoomedView )
{
	if( !g_pGameRules )
		return;
    ...
    ITexture *pRenderTarget = GetZoomTexture( );
    Assert( !IsErrorTexture( pRenderTarget ) );
    ...
    monitorView.zFar = 32768.0;
 	...
 	render->Push3DView( monitorView, VIEW_CLEAR_DEPTH, true, pRenderTarget, m_Frustum );

	int iClearFlags = 0;
	bool bDrew3dSkybox = false;
	bool bSkyboxVisible = false;

	Draw3dSkyboxworld( monitorView, iClearFlags, bDrew3dSkybox, bSkyboxVisible );
	ViewDrawScene( bDrew3dSkybox, bSkyboxVisible, monitorView, 0, VIEW_MONITOR );
    ...
    materials->DrawScreenSpaceQuad( mMask );
