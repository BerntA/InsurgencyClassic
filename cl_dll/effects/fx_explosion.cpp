...
void C_BaseExplosionEffect::CreateCore( void )
{
	...
	Vector vecEffectDir = m_vecDirection * 8;
	...
	if( tr.DidHitWorld( ) )
	{
		...
		bLargeDust = ( pData->game.material == CHAR_TEX_CONCRETE || 
	...
	if ( (m_fFlags & TE_EXPLFLAG_NOFIREBALLSMOKE) == 0 )
	{
		...
		for ( i = 0; i < 4; i++ )
		{
			...
			if ( pParticle != NULL )
			{
				...
				if( bLargeDust )
				{
					colorRamp = min( 255, random->RandomInt( luminosity, luminosity * 1.25f ) );
					pParticle->m_uchColor[0] = color[0] * colorRamp;
					pParticle->m_uchColor[1] = color[1] * colorRamp;
					pParticle->m_uchColor[2] = color[2] * colorRamp;
				}
				else
				{
		...
		for ( i = 0; i < 8; i++ )
		{
			...
			if ( pParticle != NULL )
			{
				...
				if( bLargeDust )
					...
					pParticle->m_flDieTime	= random->RandomFloat( 1.5f, 2.0f );
		...
		for ( i = 0; i < numRingSprites; i++ )
		{
			...
			if ( pParticle != NULL )
			{
				...
				if( bLargeDust )
					pParticle->m_flDieTime	= random->RandomFloat( 6.0f, 8.0f );
				else
