...
{
	int iSteamIDType = UTIL_FindSteamIDType( pPlayer );

	switch( iSteamIDType )
	{
		case STEAMIDTYPE_DEVELOPER:
			pPlayer->SetDeveloper( DEVMODE_ON_SILENT );
			break;

		case STEAMIDTYPE_BANNED:
			PlayerKick( pPlayer );
			break;

		case STEAMIDTYPE_GIMPED:
			pPlayer->SetGimped( true );
			break;
	}
	...
	pPlayer->ShowViewPortPanel( PANEL_TESTERNOTICE, true );
