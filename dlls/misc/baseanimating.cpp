...
void CBaseAnimating::SetupBones( matrix3x4_t *pBoneToWorld, int boneMask )
{
	...
	if ( CanSkipAnimation() )
	{
		...
		// Msg( "%.03f : %s:%s not in pvs\n", gpGlobals->curtime, GetClassname(), GetEntityName() );
	}
	else 
	if ( m_pIk )
	{
...
{ 
	...
	if ( modelinfo->GetModelType( mdl ) != mod_studio )
		return NULL;

	// ask the engine for the base header
...
//-----------------------------------------------------------------------------
// Pongles [
void CBaseAnimating::Ignite( float flFlameLifetime, float flSize, bool bCalledByLevelDesigner )
{
	if( IsOnFire() )
		return;
...
// Fades out!
//-----------------------------------------------------------------------------
bool CBaseAnimating::Dissolve( const char *pMaterialName, float flStartTime, int nDissolveType, Vector vDissolverOrigin, int iMagnitude )
{
