...
// Purpose: 
//-----------------------------------------------------------------------------
CBasePlayer *CPropVehicleDriveable::GetDriver( void ) 
{ 
	// Pongles [
	/*if ( m_hNPCDriver ) 
		return m_hNPCDriver; */
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::EnterVehicle( CBasePlayer *pPassenger )
{
	if ( pPassenger == NULL )
		return;
...
//-----------------------------------------------------------------------------
bool CPropVehicleDriveable::CanEnterVehicle( CBasePlayer *pEntity )
{
...
//-----------------------------------------------------------------------------
bool CPropVehicleDriveable::CanExitVehicle( CBasePlayer *pEntity )
{
...
bool CPropVehicleDriveable::NPC_AddPassenger( CAI_BaseNPC *pPassenger, string_t strRoleName, int nSeatID )
{
	...
	if ( NPC_CanEnterVehicle( pPassenger, true ) == false ) // FIXME
		return false;
...
bool CPropVehicleDriveable::NPC_RemovePassenger( CAI_BaseNPC *pPassenger )
{
	...
	if ( NPC_CanExitVehicle( pPassenger, true ) == false ) // FIXME
		return false;
...
void CFourWheelServerVehicle::GetVehicleViewPosition( int nRole, Vector *pAbsOrigin, QAngle *pAbsAngles )
{
	...
	CBasePlayer *pPassenger = GetPassenger( VEHICLE_ROLE_DRIVER );
	Assert( pPassenger );
