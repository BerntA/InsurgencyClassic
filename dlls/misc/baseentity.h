...
public:
	...
	virtual bool					IsRagDoll( void ) const { return false; }
	...
	*/
	...
	//virtual int GetFrags() const { return m_iFrags; }
	//virtual int	GetDeaths() const { return m_iDeaths; }

	//virtual void	ResetFrags();
	//virtual void	IncrementFrags( int nCount );

	//virtual void	ResetDeaths();
	//virtual void	IncrementDeaths( int nCount );
	// Pongles ]
...
public:
	...
	virtual const Vector &GetViewOffset() { return m_vecViewOffset.Get(); }
	const Vector &GetViewOffset() const { return const_cast<CBaseEntity *>(this)->GetViewOffset(); }
	void			SetViewOffset( const Vector &vecOffset );
	void			SetViewOffsetX( float x );
	void			SetViewOffsetY( float y );
...
private:
	...
	bool					ComputeStepSimulationNetworkOrigin( StepSimulationData *step );
	bool					ComputeStepSimulationNetworkAngles( StepSimulationData *step );
public:
...
inline const QAngle& CBaseEntity::GetAbsAngles( void ) const
{
	...
	if (IsEFlagSet(EFL_DIRTY_ABSTRANSFORM))
	{
		//Msg("FUCKFUCKFUCKFUCKFUCK!\n");
		const_cast<CBaseEntity*>(this)->CalcAbsolutePosition();
	}
	//Msg("SERVER: %f,%f,%f\n",m_angAbsRotation[0],m_angAbsRotation[1],m_angAbsRotation[2]);
