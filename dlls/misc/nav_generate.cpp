...
void CNavArea::ComputeApproachAreas( void )
{
	...
	// adamjmac [
	// I don't need these (yet?), so don't generate them. They take forever...
	return;
	// ] adamjmac
...
void CNavMesh::BeginGeneration( bool incremental )
{
	...
	// adamjmac [
	// because INS maps may have spawn points not accessible to the other team,
	// sampling will done from walkable nodes at ALL of the player spawn points,
	// instead of just one.
	...
	do {
		...
		spawn = gEntList.FindEntityByClassname( spawn, "ins_spawnpoint" );
		if (!spawn) break;
	...
	Msg("%d player spawnpoints found for sampling.\n", num_spawns);
