	...
	float flStartTime, int nDissolveType, bool *pRagdollCreated )
{
	...
	if ( pTarget->IsPlayer( ) && pTarget->m_lifeState == LIFE_ALIVE )
	{
		// Simply immediately kill the player.
		CBasePlayer *pPlayer = assert_cast< CBasePlayer* >( pTarget );

		CTakeDamageInfo KilledDamage( pPlayer, pPlayer, 0, DMG_GENERIC | DMG_REMOVENORAGDOLL | DMG_PREVENT_PHYSICS_FORCE );

		pPlayer->Event_Killed( KilledDamage );
		pPlayer->Event_Dying();
