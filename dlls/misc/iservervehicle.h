...
public:
	...
	virtual void			HandlePassengerEntry( CBasePlayer *pPassenger, bool bAllowEntryOutsideZone = false ) = 0;
	virtual bool			HandlePassengerExit( CBasePlayer *pPassenger ) = 0;
	...
	/*virtual int				NPC_GetAvailableSeat( CBasePlayer *pPassenger, string_t strRoleName, VehicleSeatQuery_e nQueryType ) = 0;
	virtual bool			NPC_AddPassenger( CBasePlayer *pPassenger, string_t strRoleName, int nSeat ) = 0;
	virtual bool			NPC_RemovePassenger( CBasePlayer *pPassenger ) = 0;
	virtual bool			NPC_GetPassengerSeatPosition( CBasePlayer *pPassenger, Vector *vecResultPos, QAngle *vecResultAngle ) = 0;
	virtual bool			NPC_GetPassengerSeatPositionLocal( CBasePlayer *pPassenger, Vector *vecResultPos, QAngle *vecResultAngle ) = 0;
	virtual int				NPC_GetPassengerSeatAttachment( CBasePlayer *pPassenger ) = 0;
	virtual bool			NPC_HasAvailableSeat( string_t strRoleName ) = 0;
	
	virtual const PassengerSeatAnims_t	*NPC_GetPassengerSeatAnims( CBasePlayer *pPassenger, PassengerSeatAnimType_t nType ) = 0;
	virtual CBasePlayer		*NPC_GetPassengerInSeat( int nRoleID, int nSeatID ) = 0;
...
public:
	// Pongles [
	virtual CBasePlayer		*GetDriver( void ) = 0;
	// Pongles ]
	...
	// Entering / Exiting
	virtual bool			CanEnterVehicle( CBasePlayer *pEntity ) = 0;
	virtual bool			CanExitVehicle( CBasePlayer *pEntity ) = 0;
	virtual void			SetVehicleEntryAnim( bool bOn ) = 0;
	virtual void			SetVehicleExitAnim( bool bOn, Vector vecEyeExitEndpoint ) = 0;
	virtual void			EnterVehicle( CBasePlayer *pPassenger ) = 0;

	virtual void			PreExitVehicle( CBasePlayer *pPassenger, int nRole ) = 0;
	virtual void			ExitVehicle( int nRole ) = 0;
	virtual bool			AllowBlockedExit( CBasePlayer *pPassenger, int nRole ) = 0;
	virtual bool			AllowMidairExit( CBasePlayer *pPassenger, int nRole ) = 0;
};
