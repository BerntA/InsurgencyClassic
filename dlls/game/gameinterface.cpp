...
void CServerGameDLL::ServerActivate( edict_t *pEdictList, int edictCount, int clientMax )
{
	...
	// loading the navmesh is now handled by Bot_Initialize
	// TheNavMesh->Load();
...
void CServerGameDLL::GameFrame( bool simulating )
{
	...
	// this is called by Bot_Update() now, which is called in GameStartFrame above
	//TheNavMesh->Update();
...
{
	Q_snprintf( text, maxlength, "%-64.64s", STRING( gpGlobals->mapname ) );
}
...
void CServerGameDLL::LoadMessageOfTheDay( void )
{
	CINSRules::LoadMessageOfTheDay( );
}
