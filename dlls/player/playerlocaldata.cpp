...
BEGIN_SEND_TABLE_NOBASE( CPlayerLocalData, DT_Local )
	...
	SendPropInt( SENDINFO( m_iScopeFOV ), 8, SPROP_UNSIGNED ),
	...
	SendPropVector( SENDINFO( m_vecRecoilPunchAngle ), -1, SPROP_COORD | SPROP_CHANGES_OFTEN ),
	SendPropVector( SENDINFO( m_vecRecoilPunchAngleVel ), -1, SPROP_COORD ),
...
void CPlayerLocalData::UpdateAreaBits( CBasePlayer *pl, unsigned char chAreaPortalBits[ MAX_AREA_PORTAL_STATE_BYTES ] )
{
	...
	for( int i = 0; i < MAX_AREA_PORTAL_STATE_BYTES; i++ )
	{
