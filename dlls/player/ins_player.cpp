...
//=========================================================
//=========================================================
void *SendProxy_SendTrackIRDataTable( const SendProp *pProp, const void *pStruct, const void *pVarData, CSendProxyRecipients *pRecipients, int objectID )
{
	CINSPlayer *pPlayer = ( CINSPlayer* )pStruct;

	pRecipients->ClearAllRecipients( );

	if( pPlayer->UsingTrackIR( ) )
	{
...
REGISTER_SEND_PROXY_NON_MODIFIED_POINTER( SendProxy_SendTrackIRDataTable );
...
IMPLEMENT_SERVERCLASS_ST( CINSPlayer, DT_INSPlayer )
	...
	// trackir data
	SendPropBool( SENDINFO( m_bUsingTrackIR ) ),
	SendPropDataTable( SENDINFO_DT( m_TrackIR ), &REFERENCE_SEND_TABLE( DT_INSPlayerTrackIR ), SendProxy_SendTrackIRDataTable ),
...
void CINSPlayer::PlayerRunCommand( CUserCmd *ucmd, IMoveHelper *moveHelper )
{
	...
	m_bUsingTrackIR = ucmd->trackir;

	if( m_bUsingTrackIR )
	{
		m_TrackIR.m_angHeadAngles = ucmd->headangles;
		m_TrackIR.m_flLean = ucmd->lean;
	}
...
bool CINSPlayer::ClientCommand( const char *pszCommand )
{
	...
	else if( stricmp( pszCommand, "developermode" ) == 0 ) // change developermode
	{
		switch(m_iDeveloperMode)
		{
		case DEVMODE_OFF:
			return true;
		case DEVMODE_ON_SILENT:
			SetDeveloper(DEVMODE_ON);
			break;
		case DEVMODE_ON:
			SetDeveloper(DEVMODE_ON_SILENT);
			break;
		}

		return true;
	}
	else if( stricmp( pszCommand, "devstatus" ) == 0 )
	{
		...
		switch( m_iDeveloperMode )
		{
			case DEVMODE_OFF:
			{
				pszMessage = "Not a Developer\n";
				break;
			}

			case DEVMODE_ON:
			{
				pszMessage = "Marked Developer\n";
				break;
			}

			case DEVMODE_ON_SILENT:
			{
				pszMessage = "Unmarked Developer\n";
				break;
			}
		}

		ClientPrint( this, HUD_PRINTCONSOLE, pszMessage );
...
bool CINSPlayer::BumpWeapon( CBaseCombatWeapon *pWeapon, bool bCheckVisible )
{
	...
	if( Weapon_Owns( pWeapon->GetWeaponID( ) ) )
	{
...
//=========================================================
//=========================================================
void CINSPlayer::SetDeveloper(int iMode)
{
    // Wait a moment...  What the hell?
	m_iDeveloperMode = iMode;
	PlayerResource()->UpdatePlayerDeveloper(entindex(), iMode);
}
...
void CC_Player_ForceDeveloper2(void)
{
	CINSPlayer *pPlayer = ToINSPlayer(UTIL_GetCommandClient());

	if( pPlayer )
		pPlayer->SetDeveloper( DEVMODE_ON );
}

void CC_Player_ForceDeveloper3( void )
{
    CINSPlayer *pPlayer = ToINSPlayer(UTIL_GetCommandClient());

	if( pPlayer )
		pPlayer->SetDeveloper( DEVMODE_ON_SILENT );
}

static ConCommand forcedevmode2( "forcedevmode2", CC_Player_ForceDeveloper2, "Forces the developer mode" );
static ConCommand forcedevmode3( "forcedevmode3", CC_Player_ForceDeveloper3, "Forces the developer mode (silent)" );
