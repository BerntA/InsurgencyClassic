...
ConVar perfectshot( "sv_perfectshot", "0", FCVAR_REPLICATED, "perfect shot value", true, 0, true, 2 );

#endif

#ifdef _DEBUG

ConVar penetrationsize( "sv_penetrationsize", "1", FCVAR_REPLICATED, "magnitude of penetration" );
ConVar tracerspam( "sv_tracerspam", "0", FCVAR_REPLICATED );
...
void UTIL_FireBullet( IFireBullets *pWeapon, FireBulletsData_t &FireBulletsData, CTakeDamageInfo *pDamage, ITraceFilter &filter, trace_t &tr, bool bUseHull )
{
	...
	vecDir = manip.ApplySpread( perfectshot.GetBool( ) ? vec3_origin : pWeapon->GetSpread( ) );
	...
	while( true )
	{
		...
		flDistPenetration *= penetrationsize.GetFloat( );
...
void UTIL_FireBullets( IFireBullets *pWeapon, const Vector &vecOrigin, const Vector &vecDir, int iTracerType )
{
	...
	// force tracer type
	if( tracerspam.GetBool( ) && !UTIL_VisibleTracer( iTracerType ) )
		iTracerType = TRACERTYPE_RED;
