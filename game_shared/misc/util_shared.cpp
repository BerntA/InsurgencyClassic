...
//-----------------------------------------------------------------------------
bool CTraceFilterNoPlayers::ShouldHitEntity( IHandleEntity *pServerEntity, int contentsMask )
{
	if ( CTraceFilterSimple::ShouldHitEntity(pServerEntity, contentsMask) )
	{
		CBaseEntity *pEntity = EntityFromEntityHandle( pServerEntity );
...
{
    return dynamic_cast<CBaseEntity *>( ClientEntityList().GetEnt( entindex ) );
}

#endif

// generate an alphanumberic string
void UTIL_RandomString(char *pszBuffer, int iLength)
{
	int iCharNum;

	for(int i = 0; i < iLength; i++)
	{
		iCharNum = random->RandomInt(0, 122);

		if(48 > iCharNum)
			iCharNum += 48;
		if((57 < iCharNum) && (65 > iCharNum))
			iCharNum += 7;
		if(( 90 < iCharNum) && (97 > iCharNum))
			iCharNum += 6;

		pszBuffer[i] = (char)iCharNum;
	}
}

bool UTIL_GetCRC32File(const char *pszPath, unsigned int &iCRC32)
{
	// open and load file
	int iLength;
	byte *pBuffer = UTIL_LoadFileForMe(pszPath, &iLength);
	...
	CRC32_ProcessBuffer(&CRC32Value, (void*)pBuffer, iLength);
	CRC32_Final(&CRC32Value);

	iCRC32 = (unsigned int)(CRC32Value);
