...
// PNOTE: added in gravity fix from SDK_Known_Issues_List on wiki

//=========================================================
//=========================================================
inline void UpdatePhysicsGravity( float flGravity )
{
	if( physenv )
		physenv->SetGravity( Vector( 0, 0, -flGravity ) );
}

static void GravityChanged_Callback( ConVar *pVar, const char *pOldString )
{
#ifndef CLIENT_DLL

	UpdatePhysicsGravity( pVar->GetFloat( ) );

	if( gpGlobals->mapname != NULL_STRING )
	{
		IGameEvent *pEvent = gameeventmanager->CreateEvent( "gravity_change" );

		if( pEvent )
		{
			pEvent->SetFloat( "newgravity", pVar->GetFloat( ) );
...
public:
	...
	{
		UpdatePhysicsGravity( pEvent->GetFloat( "newgravity" ) );
	}
};

CGravityChange g_GravityChange;

#endif

ConVar sv_gravity( "sv_gravity", INS_DEFAULT_GRAVITY, FCVAR_NOTIFY | FCVAR_REPLICATED, "World Gravity Value", GravityChanged_Callback );
