...
void CUserMessages::HookMessage( const char *name, pfnUserMsgHook hook )
{
	...
	// PNOTE: ClientModeShared::Init appears to Init twice

	if( m_UserMessages[ idx ]->clienthooks.Find( hook ) != m_UserMessages[ idx ]->clienthooks.InvalidIndex( ) )
		return;
...
bool CUserMessages::DispatchUserMessage( int msg_type, bf_read &msg_data )
{
	...
	if ( entry->clienthooks.Count() == 0 )
	{
		...
		// Pongles [
		AssertMsg( 0, GetUserMessageName( msg_type ) );
		// Pongles ]
		return false;
	}
