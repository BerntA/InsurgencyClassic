...
ConVar alltalk( "sv_alltalk", "0", FCVAR_NOTIFY, "Specifies if the Teams can Hear each Other", true, 0, true, 1 );
		...
		if( g_PlayerModEnable[iClient] )
		{
			...
			for(int iOtherClient=0; iOtherClient < m_nMaxPlayers; iOtherClient++)
			{
				...
				if(pEnt && pEnt->IsPlayer() && pPlayer->OnPlayTeam() && pPlayer->IsValidSquad() && 
					(m_pHelper->CanPlayerHearPlayer(pPlayer, ToINSPlayer(pEnt))) )
				{
