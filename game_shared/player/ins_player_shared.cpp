...
ConVar debugmuzzledraw( "cl_debug_muzzledraw", "0", FCVAR_CHEAT | FCVAR_REPLICATED );
...
{
	// don't allow buttons when using TrackIR
	if( m_bUsingTrackIR )
		return;
...
//=========================================================
//=========================================================
bool CINSPlayer::UsingTrackIR( void ) const
{
	return m_bUsingTrackIR;
}
...
const QAngle &CINSPlayer::HeadAngles( void ) const
{
	...
	return m_TrackIR.m_angHeadAngles.Get( );

#else

	return m_TrackIR.m_angHeadAngles;
...
void CINSPlayer::CalcPlayerLean( Vector &eyeOrigin, QAngle &eyeAngles )
{
	...
	float flLean = m_bUsingTrackIR ? m_TrackIR.m_flLean : m_INSLocal.m_flLean;
...
void UTIL_DrawDebugMuzzle( Vector &vecMuzzle, QAngle &angMuzzle )
{
	if( !debugmuzzledraw.GetBool( ) )
		return;

	Vector vecMuzzleEnd, vecDir;
	AngleVectors( angMuzzle, &vecDir );
	VectorMA( vecMuzzle, 20, vecDir, vecMuzzleEnd );

#ifdef GAME_DLL

	NDebugOverlay::Line( vecMuzzle, vecMuzzleEnd, 0, 0, 255, true, 0.5f );

#else

	debugoverlay->AddLineOverlay( vecMuzzle, vecMuzzleEnd, 255, 0, 0, true, 0.5f );
