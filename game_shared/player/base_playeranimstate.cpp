...
void CBasePlayerAnimState::ComputeMainSequence()
{
	...
	/* int animDesired = pPlayer->SelectWeightedSequence( ACT_DIERAGDOLL );
    // Assert( TranslateActivity( idealActivity ) != idealActivity );
	...
	*bIsMoving = false;
	...
	if ( isMoving && CanThePlayerMove() )
	{
		...
		else
		{
			...
			// flReturnValue = clamp( flReturnValue, 0.01, 10 );	// don't go nuts here.  // Why the hell not?
		}
		*bIsMoving = true;
	}
...
#ifndef CLIENT_DLL
		//Adrian: Make the model's angle match the legs so the hitboxes match on both sides.
		// deathz0rz: this fucks up prone body angles
		// GetOuter()->SetLocalAngles( QAngle( 0, m_flCurrentFeetYaw, 0 ) );
#endif

		int iMoveX = GetOuter()->LookupPoseParameter( pStudioHdr, "move_y" );
		int iMoveY = GetOuter()->LookupPoseParameter( pStudioHdr, "move_x" );
