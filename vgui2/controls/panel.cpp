...
// Pongles [
void Panel::CopyBounds(Panel *pPanel)
{
	int iXPos, iYPos, iWide, iTall;
	pPanel->GetBounds(iXPos, iYPos, iWide, iTall);

	SetBounds(iXPos, iYPos, iWide, iTall);
}
// Pongles ]
...
void Panel::ApplySettings(KeyValues *inResourceData)
{
	...
	bool bYPutInStorage = false;

	if (ystr)
	{
		// look for alignment flags
		if (ystr[0] == 'b' || ystr[0] == 'B')
		{
			bYPutInStorage = true;
